...
class TestSettledMarket:
    ...
    def test_settled_rows(self, page: Page, create_settled_market):
        ...
        # 6001-MARK-015
        # 6001-MARK-016
        expect(row_selector.locator('[col-id="settlementDataOracleId"]')).to_have_text("110.00")
        expected_pattern = re.compile(r"https://.*?/oracles/[a-f0-9]{64}")
        actual_href = row_selector.locator('[col-id="settlementDataOracleId"] [data-testid="link"]').get_attribute("href")
        assert expected_pattern.match(actual_href), f"Expected href to match {expected_pattern.pattern}, but got {actual_href}"
...
def test_no_closed_markets( page: Page):
        page.goto(f"/#/markets/all")
        page.get_by_test_id("Closed markets").click()
        expect(page.get_by_text("No markets")).to_be_visible()
