...
def test_trading_chart(continuous_market, vega: VegaService, page: Page):
    ...
    overlay_info = [
        ...
        InfoItem(
            ...
            "Price Monitoring Bounds 1: Min 83.11038Max 138.66685Reference 107.50000",
        ),
    ]
    # 6004-CHAR-004
    # 6004-CHAR-008
    ...
    expected_ohlc = (
        "O 101.50000H 101.50000L 101.50000C 101.50000Change −6.00000(−5.58%)"
    )
...
def check_menu_items(page, trigger_selector, valid_texts, click_item=None):
    ...
    assert len(items) == len(
        valid_texts
    ), f"Expected {len(valid_texts)} items but found {len(items)} items."

    for i, el in enumerate(items):
        text = el.text_content().strip()
        assert (
            text == valid_texts[i]
        ), f"Expected text '{valid_texts[i]}' but found '{text}'."
    if click_item:
        page.click(f"div[role='menuitemradio']:text-is('{click_item}')")
        page.click(trigger_selector)
        checked_item_text = page.text_content(
            "div[role='menuitemradio'][data-state='checked']"
        ).strip()
        assert (
            checked_item_text == click_item
        ), f"Expected checked item text '{click_item}' but found '{checked_item_text}'."
    page.click(trigger_selector, force=True)
...
def check_menu_item_checkbox(page, button_text, items):
    ...
    assert len(checkbox_items) == len(
        items
    ), f"Expected {len(items)} checkboxes but found {len(checkbox_items)}."

    for i, el in enumerate(checkbox_items):
        text = el.text_content().strip()
        assert (
            text == items[i].name
        ), f"Expected checkbox text '{items[i].name}' but found '{text}'."
